...
n_cont = 30;
n_run_ref = 20; % for runing random DOEs
mode = 3; % for call benchmark
name_1 = 'Rand';
name_2 = 'LHS';
name_3 = 'Sobol';
name_4 = 'A_LHS';
...
% Compute the expectation intergration;
if mode == 1 || mode == 2 || mode == 3
    F_ref = zeros(n_neck*NC,1);
    for i = 1:n_neck
        F_ref((i-1)*NC+1:i*NC) = kernel_benchmark(X_mean(i,:),n_neck,i,mode);
    end
else
    F_ref = zeros(n_neck,1);
    for i = 1:n_neck
        F_ref(i) = kernel_benchmark(X_mean(i,:),n_neck,i,mode);
    end
end
...
for i = 1:n_run_ref
    for h = 1:5:n_long
        ...
        X_mean_Rand = subgroup_and_compute_average(NC,NB,Pick_Rand,Neck);
        X_mean_LHS = subgroup_and_compute_average(NC,NB,Pick_LHS,Neck);
        %%
        ...
        if mode == 1 || mode == 2 || mode == 3
            F_Rand = zeros(n_neck*NC,1);
            F_LHS = zeros(n_neck*NC,1);
            %%
            ...
            for k = 1:n_neck
                F_Rand((k-1)*NC+1:k*NC) = kernel_benchmark(X_mean_Rand(k,:),n_neck,k,mode);
                F_LHS((k-1)*NC+1:k*NC) = kernel_benchmark(X_mean_LHS(k,:),n_neck,k,mode);
                %%
                F_Sobol((k-1)*NC+1:k*NC) = kernel_benchmark(X_mean_Sobol(k,:),n_neck,k,mode);
                F_A_LHS((k-1)*NC+1:k*NC) = kernel_benchmark(X_mean_A_LHS(k,:),n_neck,k,mode);
                F_MDS((k-1)*NC+1:k*NC) = kernel_benchmark(X_mean_MDS(k,:),n_neck,k,mode);
                F_A_Greedy((k-1)*NC+1:k*NC) = kernel_benchmark(X_mean_A_Greedy(k,:),n_neck,k,mode);
            end
        else
            F_Rand = zeros(n_neck,1);
            F_LHS = zeros(n_neck,1);
            %%
            ...
            for k = 1:n_neck
                F_Rand(k) = kernel_benchmark(X_mean_Rand(k,:),n_neck,k,mode);
                F_LHS(k) = kernel_benchmark(X_mean_LHS(k,:),n_neck,k,mode);
                %%
                F_Sobol(k) = kernel_benchmark(X_mean_Sobol(k,:),n_neck,k,mode);
                F_A_LHS(k) = kernel_benchmark(X_mean_A_LHS(k,:),n_neck,k,mode);
                F_MDS(k) = kernel_benchmark(X_mean_MDS(k,:),n_neck,k,mode);
                F_A_Greedy(k) = kernel_benchmark(X_mean_A_Greedy(k,:),n_neck,k,mode);
            end
        end
        ...
        if mode == 1 || mode == 2 || mode == 3
            for k = 1:n_neck
                Err_Rand = Err_Rand + sqrt((1/NC*sum((F_ref((k-1)*NC+1:k*NC)-F_Rand((k-1)*NC+1:k*NC)).^2)));
                Err_LHS = Err_LHS + sqrt((1/NC*sum((F_ref((k-1)*NC+1:k*NC)-F_LHS((k-1)*NC+1:k*NC)).^2)));
                %%
                Err_Sobol = Err_Sobol + sqrt((1/NC*sum((F_ref((k-1)*NC+1:k*NC)-F_Sobol((k-1)*NC+1:k*NC)).^2)));
                Err_A_LHS = Err_A_LHS + sqrt((1/NC*sum((F_ref((k-1)*NC+1:k*NC)-F_A_LHS((k-1)*NC+1:k*NC)).^2)));
                Err_MDS = Err_MDS + sqrt((1/NC*sum((F_ref((k-1)*NC+1:k*NC)-F_MDS((k-1)*NC+1:k*NC)).^2)));
                Err_A_Greedy = Err_A_Greedy + sqrt((1/NC*sum((F_ref((k-1)*NC+1:k*NC)-F_A_Greedy((k-1)*NC+1:k*NC)).^2)));
            end
        else
            for k = 1:n_neck
                Err_Rand = Err_Rand + sqrt((1/NC*sum((F_ref(k)-F_Rand(k)).^2)));
                Err_LHS = Err_LHS + sqrt((1/NC*sum((F_ref(k)-F_LHS(k)).^2)));
                %%
                Err_Sobol = Err_Sobol + sqrt((1/NC*sum((F_ref(k)-F_Sobol(k)).^2)));
                Err_A_LHS = Err_A_LHS + sqrt((1/NC*sum((F_ref(k)-F_A_LHS(k)).^2)));
                Err_MDS = Err_MDS + sqrt((1/NC*sum((F_ref(k)-F_MDS(k)).^2)));
                Err_A_Greedy = Err_A_Greedy + sqrt((1/NC*sum((F_ref(k)-F_A_Greedy(k)).^2)));
            end
        end
...
figure(1) 
errorbar(nDOE(1:5:n_long),Rand_mean(1:5:n_long),Rand_std(1:5:n_long),'m')
hold on
errorbar(nDOE(1:5:n_long),LHS_mean(1:5:n_long),LHS_std(1:5:n_long),'c')
hold on
errorbar(nDOE(1:5:n_long),Sobol_mean(1:5:n_long),Sobol_std(1:5:n_long),'k')
hold on
errorbar(nDOE(1:5:n_long),A_LHS_mean(1:5:n_long),A_LHS_std(1:5:n_long),'g')
hold on
errorbar(nDOE(1:5:n_long),MDS_mean(1:5:n_long),MDS_std(1:5:n_long),'b')
hold on
errorbar(nDOE(1:5:n_long),A_Greedy_mean(1:5:n_long),A_Greedy_std(1:5:n_long),'r')

legend('R-Random','R-LHS','A-Sobol','A-LHS','Greedy-MDS','Adapted-Greedy')

figure(2) 
errorbar(nDOE(1:5:n_long),Rand_median(1:5:n_long),Rand_std(1:5:n_long),'m')
hold on
errorbar(nDOE(1:5:100),LHS_median(1:5:n_long),LHS_std(1:5:n_long),'c')
hold on
errorbar(nDOE(1:5:n_long),Sobol_median(1:5:n_long),Sobol_std(1:5:n_long),'k')
hold on
errorbar(nDOE(1:5:n_long),A_LHS_median(1:5:n_long),A_LHS_std(1:5:n_long),'g')
hold on
errorbar(nDOE(1:5:n_long),MDS_median(1:5:n_long),MDS_std(1:5:n_long),'b')
hold on
errorbar(nDOE(1:5:n_long),A_Greedy_median(1:5:n_long),A_Greedy_std(1:5:n_long),'r')
legend('R-Random','R-LHS','A-Sobol','A-LHS','Greedy-MDS','Adapted-Greedy')

figure(3) % plot in logscale
errorbar(nDOE(1:5:n_long),log(Rand_mean(1:5:n_long)),(Rand_std(1:5:n_long)),'m')
hold on
errorbar(nDOE(1:5:n_long),log(LHS_mean(1:5:n_long)),(LHS_std(1:5:n_long)),'c')
hold on
errorbar(nDOE(1:5:n_long),log(Sobol_mean(1:5:n_long)),Sobol_std(1:5:n_long),'k')
hold on
errorbar(nDOE(1:5:n_long),log(A_LHS_mean(1:5:n_long)),A_LHS_std(1:5:n_long),'g')
hold on
errorbar(nDOE(1:5:n_long),log(MDS_mean(1:5:n_long)),MDS_std(1:5:n_long),'b')
hold on
errorbar(nDOE(1:5:n_long),log(A_Greedy_mean(1:5:n_long)),A_Greedy_std(1:5:n_long),'r')
legend('R-Random','R-LHS','A-Sobol','A-LHS','Greedy-MDS','Adapted-Greedy')

figure(4)
plot(nDOE(1:5:n_long),log(Rand_median(1:5:n_long)),'m')
hold on
plot(nDOE(1:5:n_long),log(LHS_median(1:5:n_long)),'c')
hold on
plot(nDOE(1:5:n_long),log(Sobol_median(1:5:n_long)),'k')
hold on
plot(nDOE(1:5:n_long),log(A_LHS_median(1:5:n_long)),'g')
hold on
plot(nDOE(1:5:n_long),log(MDS_median(1:5:n_long)),'b')
hold on
plot(nDOE(1:5:n_long),log(A_Greedy_median(1:5:n_long)),'r')
legend('R-Random','R-LHS','A-Sobol','A-LHS','Greedy-MDS','Adapted-Greedy')
